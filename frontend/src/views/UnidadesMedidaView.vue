...
const form = ref({ id: null, descripcion: '', estado: 'Activo' })
...
const openModal = (item = null) => {
  if (item) {
    ...
    form.value = { id: null, descripcion: '', estado: 'Activo' }
  }
...
const save = async () => {
  if (isEditing.value) {
    await store.editUnidad(form.value.id, form.value)
  } else {
    await store.addUnidad({ descripcion: form.value.descripcion, estado: form.value.estado })
  }
  closeModal()
}
...
<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-gray-800">Unidades de Medida</h1>
      <button @click="openModal()" class="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg font-medium transition-colors shadow-sm">
        <Plus class="w-5 h-5" /> Nueva Unidad
      </button>
    </div>
    ...
    <div v-else class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <table class="w-full text-left border-collapse">
        ...
        <tbody>
          ...
          <tr v-if="unidades.length === 0"><td colspan="4" class="py-8 text-center text-gray-500">No hay unidades registradas.</td></tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
          <h3 class="text-lg font-bold text-gray-800">{{ isEditing ? 'Editar Unidad' : 'Nueva Unidad' }}</h3>
